...
public class SettingsManager
{
	...
	private static void prepareConfigFile()
	{
		...
		try
		{
			...
			URL resource = cl.getResource("assets/" + modid + "/presets");
			if(resource != null)
			{
				for(Path p : Files.walk(Paths.get(resource.toURI())).toList())
				{
					try
					{
						if(p.toFile().isFile())
						{
							Files.copy(p, FabricLoader.getInstance().getConfigDir().resolve(modid + "/presets/" + p.getFileName()));
						}
					}
			...
			new File(FabricLoader.getInstance().getConfigDir().toFile(), modid + "/presets").mkdirs();
			Files.copy(FabricLoader.getInstance().getConfigDir().resolve(modid + "/presets/default.txt"), file.toPath());
		}
		catch(Exception e)
		{
			e.printStackTrace();
	...
	public static void save()
	{
		...
		try
		{
			...
			for(SettingsCategory cat : Settings.category.getEnumConstants())
			{
